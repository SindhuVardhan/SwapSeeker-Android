...
class SignupActivity : AppCompatActivity() {
    ...
    private fun signupUser(username: String, password: String, phone: String, email: String) {
        // Assuming "users" is the key for user data in your database
        ...
        usersReference.addListenerForSingleValueEvent(object : ValueEventListener {
            ...
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                ...
                val userId = dataSnapshot.childrenCount + 1

                // Check if the username is already taken
                if (!dataSnapshot.hasChild(username)) {
                    // Create a new UserData object
                    val userData = UserData(userId.toString(), username, password, phone, email)

                    // Set the user data with the custom numerical ID
                    usersReference.child(username).setValue(userData)
